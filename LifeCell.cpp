...
void LifeCell_t::clock_lo ( dat_t<1> reset ) {
  ...
  val_t LifeCell__neighbor_sum;
  { LifeCell__neighbor_sum = LifeCell__sum4+LifeCell__sum5;}
  LifeCell__neighbor_sum = LifeCell__neighbor_sum & 0xfL;
  val_t T0;
  T0 = LifeCell__neighbor_sum == 0x3L;
  val_t T1;
  T1 = LifeCell__neighbor_sum == 0x2L;
  val_t T2;
  { T2 = T1 | T0;}
  val_t T3;
  { T3 = TERNARY_1(LifeCell__is_alive.values[0], T2, LifeCell__is_alive.values[0]);}
  val_t T4;
  T4 = LifeCell__neighbor_sum == 0x3L;
  val_t T5;
  { T5 = LifeCell__is_alive.values[0] ^ 0x1L;}
  val_t T6;
...
void LifeCell_t::clock_hi ( dat_t<1> reset ) {
  dat_t<1> LifeCell__is_alive__shadow = T7;
  LifeCell__is_alive = T7;
}
...
void LifeCell_api_t::init_sim_data (  ) {
  ...
  assert(mod);
  sim_data.inputs.push_back(new dat_api<1>(&mod->LifeCell__io_running));
  ...
  sim_data.signal_map["LifeCell.is_alive"] = 0;
